...
#include <frsm/PixelMap.hpp>

#include "pixel_map_renderers.h"
...
static void upload_map_texture(PixelMapRenderer *self);
...
static void pixel_map_handler(const lcm_recv_buf_t *rbuf, const char *channel, const frsm_pixel_map_t *msg,
    void *user)
{
  PixelMapRenderer *self = (PixelMapRenderer*) user;
  if (self->pix_map_msg != NULL)
    frsm_pixel_map_t_destroy(self->pix_map_msg);
  self->pix_map_msg = frsm_pixel_map_t_copy(msg);
...
static void upload_map_texture(PixelMapRenderer *self)
{
  ...
  if (data_type == 0) {
    ...
    data_type = FRSM_PIXEL_MAP_T_TYPE_FLOAT;
  }
  switch (data_type) {
  case FRSM_PIXEL_MAP_T_TYPE_FLOAT:
    {
    ...
    break;
  case FRSM_PIXEL_MAP_T_TYPE_INT32:
      {
    ...
    break;
  case FRSM_PIXEL_MAP_T_TYPE_UINT8:
    {
...
{
  PixelMapRenderer *self = (PixelMapRenderer*) renderer;
...
{
  PixelMapRenderer *self = (PixelMapRenderer*) calloc(1, sizeof(PixelMapRenderer));
  ...
  frsm_pixel_map_t_subscribe(self->lc, lcm_channel, pixel_map_handler, self);
  return &self->renderer;
}

extern "C" void pixel_map_add_renderer_to_viewer(BotViewer *viewer, int render_priority,
...
extern "C" void pixel_map_add_renderer_to_viewer_lcm(BotViewer *viewer, int render_priority, lcm_t *lcm,
    const char* lcm_channel, const char* renderer_name)
{
...
{
  pixel_map_add_renderer_to_viewer(viewer, render_priority, NULL, DEFAULT_RENDERER_NAME);
}
