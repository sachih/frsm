...
int main(int argc, char ** argv)
{
  ...
  frsm_pixel_map_t * pix_map_msg = load_pixel_map_t_from_file(filename.c_str());

  Uint8PixelMap * map;
  if (pix_map_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_FLOAT) {
    FloatPixelMap * fmap = new FloatPixelMap(pix_map_msg);
    map = new Uint8PixelMap(fmap, true, floatToUint8);
    delete fmap;
  }

  else if (pix_map_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_UINT8) {
    map = new Uint8PixelMap(pix_map_msg);
  }
